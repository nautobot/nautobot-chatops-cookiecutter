...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_PLUGIN_CHATOPS_MY_PLUGIN_xxx
...
namespace = Collection("nautobot_plugin_chatops_my_plugin")
namespace.configure(
    {
...
def docker_compose(context, command, **kwargs):
    ...
    """
    build_env = {
        "NAUTOBOT_VER": context.nautobot_plugin_chatops_my_plugin.nautobot_ver,
        "PYTHON_VER": context.nautobot_plugin_chatops_my_plugin.python_ver,
    }
    compose_command = f'docker-compose --project-name {context.nautobot_plugin_chatops_my_plugin.project_name} --project-directory "{context.nautobot_plugin_chatops_my_plugin.compose_dir}"'
    for compose_file in context.nautobot_plugin_chatops_my_plugin.compose_files:
        compose_file_path = os.path.join(context.nautobot_plugin_chatops_my_plugin.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_plugin_chatops_my_plugin.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_plugin_chatops_my_plugin.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_plugin_chatops_my_plugin"
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_plugin_chatops_my_plugin'
    run_command(context, command)
...
def unittest(context, keepdb=False, label="nautobot_plugin_chatops_my_plugin", failfast=False, buffer=True):
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_plugin_chatops_my_plugin/*' --omit *migrations*"
...
def tests(context, failfast=False):
    ...
    if not is_truthy(context.nautobot_plugin_chatops_my_plugin.local):
