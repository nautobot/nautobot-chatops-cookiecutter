...
After updating the `nautobot_plugin_chatops_my_plugin/worker.py` file and saving it, the backend Django service should auto-reload with your changes. If it doesn't, or a bug in the code caused it to crash, you can quickly relaunch it from your poetry environment with `invoke restart`.
...
So for example, our plugin is named `nautobot-plugin-chatops-my-plugin`, the command would most likely be `docker logs nautobot_plugin_chatops_my_plugin_nautobot_1 -f`. You can find the name of all running containers via `docker ps`.
...
Or set the `INVOKE_NAUTOBOT_PLUGIN_CHATOPS_MY_PLUGIN_PYTHON_VER` variable
...
Or set the `INVOKE_NAUTOBOT_PLUGIN_CHATOPS_MY_PLUGIN_NAUTOBOT_VER` variable
